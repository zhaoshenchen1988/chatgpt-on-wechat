...
class Hello(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if content in ["Hi", "hi", "Hello", "hello"]:
            reply = Reply()
            reply.type = ReplyType.TEXT
            msg: ChatMessage = e_context["context"]["msg"]
            if e_context["context"]["isgroup"]:
                reply.content = f"Hi Mua~ {msg.actual_user_nickname} from {msg.from_user_nickname}"
            else:
                reply.content = f"Hi Mua~ {msg.from_user_nickname}"
            e_context["reply"] = reply
